...
public class HeroEntity extends AbstractEntity implements Controllable, Damageable {
  ...
  public HeroEntity(String imagePath, double xPos, double yPos, double width, double height, Layer layer, int lives) {
  ...
  public void takeDamage(Level level) {
    ...
    if (newLives <= 0) {
      ...
      System.out.println("=== YOU HAVE SENT BACK THE ORIGIN ! ===");
      System.out.println("    "+newLives+" LIVES LEFT !   \n");
    }
  }
  @Override
  public Entity copy(){
    Entity entity = new HeroEntity(getImagePath(), getXPos(), getYPos(), getWidth(), getHeight(), getLayer(), getLives());
    return entity;
  }
  @Override
  public int getLives() {
