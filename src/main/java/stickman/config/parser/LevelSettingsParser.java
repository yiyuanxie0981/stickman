...
public class LevelSettingsParser {
  ...
  public LevelSettings parse(JSONObject configObject) {
    ...
    Double newTargetGameTime = ConfigurationUtils.parseDoubleFromObject(settingsData, "targetGameTime");
    if (newTargetGameTime != null) {
      targetGameTime = newTargetGameTime;
    }

    return new LevelSettings(entityData, width, floorHeight,targetGameTime);
  }
}
